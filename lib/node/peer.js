...
Peer.prototype.broadcast = function broadcast(values, callback) {
  // TODO(indutny): this may be unoptimal at times as remote end may know
  // these values already
  if (values.length !== 0)
    this.stream.data({ values }, callback);
  else
...
Peer.prototype.close = function close(callback) {
  if (this.stream.close)
    return this.stream.close(callback);
  this.destroy();
  process.nextTick(callback, null);
};
